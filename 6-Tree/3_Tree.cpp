...
// Max Diameter of Binary Tree Optimised Approach
int max_dia = 0;
int maxheight(struct Node* root){
    if(root == nullptr) return 0;

    int lheight = maxheight(root->left) + 1;
    int rheight = maxheight(root->right) + 1;
    int curr_dia = lheight + rheight;
